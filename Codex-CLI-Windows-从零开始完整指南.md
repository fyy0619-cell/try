...
- [七、切换账号 / 重新登录](#七切换账号--重新登录)
- [八、第一次跑 Codex（Hello World）](#八第一次跑-codexhello-world)
- [九、日常使用：模式、模型、常用命令](#九日常使用模式模型常用命令)
- [十、配置文件 `~/.codex/config.toml`](#十配置文件-codexconfigtoml)
- [十一、常见问题与解法（FAQ）](#十一常见问题与解法faq)
- [十二、卸载与彻底清理](#十二卸载与彻底清理)
- [十三、进阶资源](#十三进阶资源)
...
如果你机器上已经装好并登录过 Codex CLI，现在想**换一个账号**——比如换另一个 ChatGPT 账号、换一个 API Key、或者从 ChatGPT 登录切换到 API Key（反之亦然）——按本章来。**不要直接卸载重装**，那是杀鸡用牛刀，而且会丢失你的 `config.toml` 和会话记录。

### 7.1 先确认你当前用的是哪种登录方式

不同的登录方式切起来步骤完全不同，先确认现状：
...
- 显示**邮箱**（如 `xxx@example.com`、`xxx@gmail.com`） → 当前是 **ChatGPT OAuth 登录**
- 显示 `via API key` / `OPENAI_API_KEY` / 没有邮箱只有模型名 → 当前是 **API Key 登录**
...
输出形如 `Authenticated as: xxx@example.com (ChatGPT Plus)` 或 `Using API key from env: OPENAI_API_KEY`。

> 如果 `codex auth status` 报 `unknown command`，说明你装的是旧版本，跳过这条直接看状态栏即可，不影响后续。

确认完后输入 `/exit` 回到 PowerShell。再根据你的目标场景跳到下面对应小节：

| 你的情况 | 跳到 |
|---|---|
| 当前 ChatGPT 登录 → 换另一个 ChatGPT 账号 | [7.2](#72-场景-a从-chatgpt-账号-a-换到-chatgpt-账号-b) |
| 当前 API Key → 换另一个 API Key | [7.3](#73-场景-b从-api-key-a-换到-api-key-b) |
| 当前 ChatGPT 登录 → 改用 API Key | [7.4](#74-场景-c从-chatgpt-登录切换到-api-key) |
| 当前 API Key → 改用 ChatGPT 登录 | [7.5](#75-场景-d从-api-key-切换到-chatgpt-登录) |
| 上面方法都没把账号切干净 | [7.6](#76-终极方案手动彻底清理登录态) |

---

### 7.2 场景 A：从 ChatGPT 账号 A 换到 ChatGPT 账号 B
...
1. **退出当前登录**：
   ...
   codex logout
   ```
   看到 `Logged out` 之类的提示即成功。

   > 如果报 `unknown command: logout`，改用：
   > ```powershell
   > codex auth logout
   > ```
   > 两条都不识别（罕见，仅老版本），跳到 [7.6 终极方案](#76-终极方案手动彻底清理登录态)。

2. **清掉浏览器对 OpenAI 的登录态**——这一步**最容易被忽略，是切账号失败的头号原因**。OAuth 会复用浏览器里 `chatgpt.com` / `auth.openai.com` 的登录 cookie，如果你没清，重新点 `Sign in with ChatGPT` 时浏览器会**直接拿账号 A 完成授权**，回到终端你以为换了，实际还是 A。

   三种清法任选一种：

   - **最省事**：用浏览器**无痕/隐私窗口**完成接下来的登录。Edge 是 `Ctrl+Shift+N`，Chrome 同样。无痕窗口完全独立的 cookie 空间。
   - **彻底清**：浏览器按 `Ctrl+Shift+Delete` → 时间范围选**全部时间** → 勾选 **Cookies 和其他网站数据** → 清除。注意这会让你**所有网站**都退出登录。
   - **精确清**：浏览器地址栏分别输入下面两个 URL，把列出来的 cookies 都删了：
     - Edge / Chrome：`chrome://settings/cookies/detail?site=chatgpt.com`
     - Edge / Chrome：`chrome://settings/cookies/detail?site=openai.com`

3. **用新账号重新登录**：
   ```powershell
   codex
   ```
   选 `Sign in with ChatGPT`。浏览器（或无痕窗口）跳到登录页时，**用账号 B 登录**并授权。

4. **验证**：再次启动 `codex`，底部状态栏邮箱应为账号 B。

---

### 7.3 场景 B：从 API Key A 换到 API Key B

**步骤：**

1. **先去 OpenAI 后台生成新 Key**：浏览器打开 `https://platform.openai.com/api-keys`，确认你登录的是**新 Key 要归属的那个组织**（右上角能切组织），点 `+ Create new secret key` → 命名 → `Create`。复制 `sk-proj-...` 这一串，**只显示这一次**。

2. **撤销旧 Key**（强烈建议）：同一页面找到旧 Key 那一行，点 `Revoke`。即使你不打算用了，留着也是泄漏风险——曾经显示过的 Key 一律按"已泄漏"对待。

3. **更新本机环境变量**（替换为新 Key）：
   ```powershell
   setx OPENAI_API_KEY "sk-新KEY"
   ```
   看到 `SUCCESS: 指定的值已得到保存` 即成功。

   > **关键：** `setx` 不会修改当前 PowerShell 窗口的环境变量，只对**之后新开的**终端生效。

4. **关掉所有 PowerShell / cmd / Git Bash 窗口，重新开一个 PowerShell**。

5. **验证环境变量**：
   ```powershell
   echo $env:OPENAI_API_KEY
   ```
   应该输出新的 `sk-...`，不是旧的。如果还是旧的，说明你没关干净所有终端，重新开。
...
> **注意：** 如果你以前是把 Key 写在 `~/.codex/config.toml` 或某个 `.env` 文件里（不推荐），还得去对应地方改。本指南推荐的方式是只走环境变量。

---

### 7.4 场景 C：从 ChatGPT 登录切换到 API Key
...
1. **退出 ChatGPT 登录**：
   ...
   codex logout
   ```
   （报错则改 `codex auth logout`）

2. **按 [7.3](#73-场景-b从-api-key-a-换到-api-key-b) 的 Step 1** 拿到一个 API Key 并充值。
...
4. **启动 codex**：
   ...
   codex
   ```
   它检测到 `OPENAI_API_KEY` 后会自动用 API Key。如果启动菜单仍弹出 `Sign in with ChatGPT` / `Use API key` 选项，选**后者**。

5. **可选——锁定提供方**避免每次问。编辑 `~/.codex/config.toml`，加（或修改成）：
   ```toml
...
### 7.5 场景 D：从 API Key 切换到 ChatGPT 登录

**关键认知：** 只要 `OPENAI_API_KEY` 环境变量还在，Codex 启动时就会优先用它，**即使你登录了 ChatGPT 账号**。所以第一步必须先删环境变量。

**步骤：**

1. **删除 `OPENAI_API_KEY` 环境变量**，两种方式任选：
   ...
   **方式 1（PowerShell 命令）**：
   ...
   > **不要用** `setx OPENAI_API_KEY ""`——它在某些 Windows 版本上**只是把值设为空串而不是真删**，Codex 仍会读到一个空字符串并报"key 无效"。用上面的 PowerShell 命令才是真删。

   **方式 2（图形界面）**：
   - `Win + R` → 输入 `sysdm.cpl` → 回车
   - 切到「高级」选项卡 → 点「环境变量」
   - 在「用户变量」列表里找到 `OPENAI_API_KEY` → 选中 → 删除 → 确定
   - 如果「系统变量」里也有同名的，也删掉

2. **关掉所有终端，重新开一个 PowerShell**。

3. **验证已删干净**：
   ```powershell
   echo $env:OPENAI_API_KEY
   ```
   应该输出**空行**（什么都没显示）。如果还显示 `sk-...`，说明你有别的窗口没关、或者「系统变量」里还有残留——回 Step 1 再清一遍。

4. **启动 codex 走 OAuth**：
   ```powershell
   codex
   ```
   选 `Sign in with ChatGPT`，浏览器登录授权。

5. **验证**：状态栏应显示 ChatGPT 邮箱，不再显示 `via API key`。

---

### 7.6 终极方案：手动彻底清理登录态

`codex logout` 没用、上面四个场景的流程都没把账号切干净、或者你想从零开始一个干净的登录环境——按下面**每一条都执行**：

1. **删除 Codex 本地凭据文件**：
   ```powershell
   Remove-Item -Force $HOME\.codex\auth.json -ErrorAction SilentlyContinue
   ```

2. **（可选）删除 Codex 会话历史**——会丢失之前所有会话记录，**确认了再删**：
   ```powershell
   Remove-Item -Recurse -Force $HOME\.codex\sessions -ErrorAction SilentlyContinue
   ```

3. **清 Windows 凭据管理器里 Codex/OpenAI 缓存的凭据**：
   - 控制面板 → 用户账户 → 凭据管理器 → 点「**Windows 凭据**」
   - 找包含 `openai` / `codex` / `chatgpt` 字样的条目，**每条点开 → 删除**
   - 切到「**Web 凭据**」，同样处理
   - 命令行快捷方式：
     ```powershell
...
4. **删 `OPENAI_API_KEY` 环境变量**（按 [7.5 Step 1](#75-场景-d从-api-key-切换到-chatgpt-登录) 操作）。

5. **清浏览器对 OpenAI 的 cookies**（按 [7.2 Step 2](#72-场景-a从-chatgpt-账号-a-换到-chatgpt-账号-b) 操作）。

6. **保留**这些（除非你想完全重置工具本身）：
   - `~/.codex/config.toml` — 你的模型、沙盒模式、profile 配置
   - 项目里的 `AGENTS.md` — 自定义指令

做完上面 1–5 步后，再 `codex` 登录，等同于一台新机器第一次登录的状态。

---

### 7.7 切换后没生效？排查清单

切完之后启动 codex 发现状态栏还是老样子，按这个表对症：

| 现象 | 大概率原因 | 解法 |
|---|---|---|
| 状态栏邮箱还是账号 A | 浏览器 cookies 没清，"重新登录"其实又登回 A | 用**无痕窗口**重新走一遍 OAuth |
| `echo $env:OPENAI_API_KEY` 还是旧 Key | 当前 PowerShell 是 `setx` **之前**打开的 | 关掉**所有**终端窗口，新开一个 |
| 切到新 Key 后报 `401 unauthorized` | 新 Key 没充值 / 已被撤销 / 复制时把首尾空格也带进去了 | platform.openai.com 检查 Key 状态；`setx` 再重新设一次（注意引号里别有空格） |
| 期望走 OAuth，但状态栏显示 `via API key` | `OPENAI_API_KEY` 环境变量没真删（可能是被设成了空串） | 走 [7.5 Step 1 方式 1](#75-场景-d从-api-key-切换到-chatgpt-登录) 重新删 |
| `codex logout` 显示成功但下次启动还是自动登录 | 凭据管理器里有缓存 / `~/.codex/auth.json` 还在 | [7.6 Step 1 + Step 3](#76-终极方案手动彻底清理登录态) |
| 切完后 codex 启动报 `Sign-in failed` 反复弹 | 浏览器 OAuth 回调被防火墙/代理拦 | 改走 API Key（[场景 C](#74-场景-c从-chatgpt-登录切换到-api-key)），不依赖回调 |
| 状态栏一切正常但跑命令时报 429 | 新账号的速率限制是单独算的，但你正好撞到限速 | 等几分钟；或检查新账号订阅状态是否真的是 Plus 及以上 |

---

### 7.8 切换账号时的安全建议

- **旧 API Key 立即撤销**：平台后台 `Revoke` 掉所有你不再用的 Key。**Key 一旦显示过就视为已泄漏**——别想着"我没贴出去过应该没事"，自动化工具、剪贴板历史、误推到 GitHub 都可能让它泄漏。
- **公司 ↔ 个人账号切换**：切完务必跑一遍 [7.6 终极清理](#76-终极方案手动彻底清理登录态)。否则下一次 codex 启动可能自动恢复到你不想要的那个账号，特别是公司账号有 SSO 自动续签的情况。
- **共享 / 公用电脑**：用完一定登出（`codex logout`）**并**删 `~/.codex/auth.json`，否则下一个使用者打开 codex 就是用的你的额度。
- **API Key 永远不要写进项目目录**：只放环境变量。`.env` 文件如果非用不可，必须加进 `.gitignore`，提交前用 `git status` 自查。
- **切完之后跑一个无害命令试探**：例如让 codex 输出 `whoami` 的执行结果，确认它真的在用新身份在跑，而不是缓存里的旧身份。

---

## 八、第一次跑 Codex（Hello World）
...
> **如果你是想主动换账号而不是修复登录失败**，请直接看 [第七章 切换账号 / 重新登录](#七切换账号--重新登录)，里面按 ChatGPT/API Key 四种切换场景分别给了步骤。
